...
# 检查状态是否已访问（通过嵌套列表比较）
def is_visited(state, visited_list):
    for visited_state in visited_list:
        if state == visited_state:  # 直接比较二维列表是否完全相同
            return True
    return False
...
def solve_puzzle(initial_state):
    ...
    while open_heap:
        ...
        for next_state in next_states:
            ...
            if not is_visited(next_state,visited):  # 若要保留更优的 g值，加上约束：   or next_g < visited[state_key]
                visited.append(next_g)
                heapq.heappush(cur_heap, (next_f, next_g, next_state))
